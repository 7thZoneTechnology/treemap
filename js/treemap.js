...
//       addClass here for css tool tip
//		.append("title")
//		.text(function(d) { 
...
/*		.classed("hint--right", true)
		.attr("data-hint", function(d) { return d.name + " " + formatNumber(d.size); 
		})
*/
	...
	g.append("rect")
		...
		.on("click", function(d) { 
			...
			if(d.children && !d.children.children) {
				return false;
			}
		})
...
//		}); /*should be d.value*/
	...
	g.append("foreignObject")
		...
		.html(function(d) { 
			if (d.size) {
				return d.name + " (" + formatNumber(d.size) + ")"; 
			} // --------------------------------------------------   is this where the last click through is happening?
...
//		if (transitioning || !d || (d.children && d.children.children)) return;
		transitioning = true;
		var g2 = display(d),
...
/*
// original function that printed out the description

function name(d) {
	return d.parent ? name(d.parent) + " in " + d.name : d.name;
}
...
function name(d) {
	...
	if( !d.parent) {
		...
		return "Reference Statistics in " + d.name;
		} else {
			return d.name + " Reference Statistics in " + d.parent.name;
		}
}
